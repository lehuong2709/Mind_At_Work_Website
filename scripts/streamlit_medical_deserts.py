...
def get_medical_deserts(state_name, n_miles, poverty_threshold=30):
    ...
    for j in range(n):
        ...
        if minimum_distance > n_miles * constants.miles_to_km and dataframe.iloc[
...
if deserts != 0:
    for race in races:
        ...
        if fractional_medical_desert_groups[race] - fractional_census_tracts[race] >= 0.10:
            st.write(race + ' population is disproportionately affected by medical deserts in ' + state_name + ', USA. ' +
                     'About :red[' + str(percent_deserts) + '%] of medical deserts are ' + race + '. ' +
                     'Only about :blue[' + str(percent_census_tracts) + '%] of Census tracts are ' + race + '.')
